...
public:
        ...
        : apvts (state), bank (b), hostBpm (bpm)
    {
        ...
        pSubLevel = p ("subLevel"); pSubOct = p ("subOct"); pNoiseLevel = p ("noiseLevel");
    ...
    void renderNextBlock (juce::AudioBuffer<float>& output, int startSample, int numSamples) override
    {
        ...
        const int   subOctave = (int) pSubOct->load() + 1; // 0 -> -1 oct, 1 -> -2 oct
        ...
        while (pos < numSamples)
        {
            ...
            for (int i = 0; i < len; ++i)
            {
                ...
                const float sub = (float) std::sin (juce::MathConstants<double>::twoPi * subPhase)
                                  * lvlSub;
                subPhase += subInc;
