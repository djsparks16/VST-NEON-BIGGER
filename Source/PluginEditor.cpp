...
namespace
{
    ...
    private:
        ...
        Knob master;
        juce::Rectangle<int> masterLabelArea;
    ...
    public:
              ...
              fx (p.apvts, juce::Colour (violet))
        {
            for (auto* c : std::initializer_list<juce::Component*> {
                     &header, &oscA, &oscB, &subNoise, &filter,
                     &env1, &env2, &lfo1, &lfo2, &voice, &matrix, &fx })
                addAndMakeVisible (*c);
        }
    ...
    : AudioProcessorEditor (&p), processor (p)
{
    ...
    setResizeLimits (1080, 720, 1920, 1200);
}
