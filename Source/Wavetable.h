...
{
public:
    enum Slot { basic = 0, pulse = 1, user = 2, numSlots };

    void init()
    {
        basicTable.buildFromFrames (makeBasicFrames());
        pulseTable.buildFromFrames (makePulseFrames());
        userTable = basicTable; // until something is loaded
    }
...
private:
    ...
    Wavetable basicTable, pulseTable, userTable;
};
...
{
public:
    enum WarpMode { warpOff = 0, warpSync, warpBend, warpMirror, warpFM };
